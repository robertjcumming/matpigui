...
% fprintf  ( 2, 'lock again\n' );  
  ...
  if options.convertLeadingSpaces
    % add user string to the output
    index = 1;
    if length ( str ) >= 1 && strcmp ( str(1), ' ' );
      leader = '';
      while true
        if length ( str ) < index ; break; end
        if strcmp( str(index), ' ' )
          leader = sprintf ( '%s&nbsp', leader );
          index = index + 1; 
          continue;
        end
        break
      ...
      str(1:index-1) = [];
      str = sprintf ( '%s;%s', leader, str );
    end
  end
  if options.convertTrailingSpaces
    % add user string to the output
    index = length(str);
    if length ( str ) > 1 && strcmp ( str(end), ' ' );
      trailer = '';
      while true
        if index < 1; break; end
        if strcmp( str(index), ' ' )
          trailer = sprintf ( '&nbsp%s', trailer );
          index = index - 1; 
          continue;
        end
        break
...
function [output, defChanged, colCode]= GetDefaults( force, item )
  ...
  if isempty ( options ) || nargin == 1 && strcmp ( force, '*reset*' )
    ...
    options.prefix = '';
    options.suffix = '';
    options.convertLeadingSpaces  = false;
    options.convertTrailingSpaces = false;
