% EXAMPLE_UIPANELDOCKING_03 - complex docking panel with callbacks
%
...
%
%    This example listens to the the undock event to allow the user to
%      update any listeners as appropriate.
%
%   see also example_uipanelDocking, example_uipanelDocking_03
%
%  Copyright Robert Cumming @ Matpi Ltd.
%  www.matpi.com
%  $Id: example_uipanelDocking_03.m 215 2015-07-27 19:00:38Z Bob $
function example_uipanelDocking_03
  ...
  for ii=1:length(names)
    ...
    myApp.addUIC ( names{ii}, mainUINames{ii},  'uipanel', 'Position', [0.0 0 0.9 1], 'BorderWidth', 0 );
    ...
    rightPanel = myApp.getPageHandle ( controlNames{ii} );
    leftPanel = myApp.getPageHandle ( mainUINames{ii} );
    % Set the pixel size.
    SetMinPixelSize ( rightPanel, leftPanel, 'right', 50 )
  end
  ...
  % add a callback function to the figure
  myApp.hFig.WindowButtonMotionFcn = @(obj,event)MyFigcallback(obj,myApp);
  
  % add a listener for the undock of a uipanel
  addlistener ( myApp, 'undock', @(obj,event)UIPanelUndocked(myApp,event) );
end
...
function UIPanelUndocked(myApp,event)
  hFig = event.data.dialog;
  hFig.WindowButtonMotionFcn = @(obj,event)MyFigcallback(hFig,myApp);
  
end
function MyFigcallback(hFig,myApp)
  hFig = handle(hFig);
  disp ( num2str(hFig.CurrentPoint) );
